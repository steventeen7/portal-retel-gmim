...
import { Mic, Plus, Save, Trash2, Edit3, XCircle, RefreshCw, ArrowLeft } from 'lucide-react';
...
export default function WawancaraAdminClient({ initialData }: { initialData: SoalWawancara[] }) {
  const router = useRouter();
  const [dataList, setDataList] = useState<SoalWawancara[]>(initialData);
  const [showForm, setShowForm] = useState(false);
  const [loading, setLoading] = useState(false);
  const [editingId, setEditingId] = useState<number | null>(null);
  
  const [form, setForm] = useState({
    kategori: 'Pengetahuan Alkitab',
    pertanyaan: '',
    jawaban_paling_tepat: '',
    penjelasan_jawaban: ''
  });

  const categories = ['Pengetahuan Alkitab', 'Sejarah GMIM', 'Ajaran Gereja', 'Keremajaan', 'Pengetahuan Umum'];

  const resetForm = () => {
    setForm({ kategori: 'Pengetahuan Alkitab', pertanyaan: '', jawaban_paling_tepat: '', penjelasan_jawaban: '' });
    setEditingId(null);
    setShowForm(false);
  };

  const handleEdit = (d: SoalWawancara) => {
    setForm({ 
      kategori: d.kategori || 'Pengetahuan Alkitab', 
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Hapus materi wawancara ini?')) return;
    try {
      ...
      <div className="flex items-center justify-between bg-white p-8 rounded-[32px] shadow-sm border border-purple-50">
        <div className="flex items-center gap-4">
           ...
           <div>
             ...
             <p className="text-gray-500 font-medium">Kelola pertanyaan dan kunci jawaban wawancara.</p>
           </div>
        </div>
        <button 
        ...
        {dataList.map(d => (
          <div key={d.id} className="bg-white rounded-[24px] p-6 border border-gray-100 shadow-sm flex flex-col gap-4 group hover:border-purple-200 transition-colors">
            <div className="flex justify-between items-start">
               <span className="text-[10px] font-black px-3 py-1 bg-purple-50 text-purple-600 rounded-full uppercase tracking-widest">{d.kategori}</span>
            ...
            <h3 className="text-lg font-bold text-gray-900 leading-snug">{d.pertanyaan}</h3>
            <div className="space-y-2 mt-2">
               <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-100">
                  <div className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-1">Jawaban Tepat</div>
                  <p className="text-sm text-emerald-800 font-medium">{d.jawaban_paling_tepat || '-'}</p>
               </div>
               {d.penjelasan_jawaban && (
                 <div className="p-4 rounded-2xl bg-amber-50 border border-amber-100">
                    <div className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-1">Penjelasan Ekstra</div>
                    <p className="text-sm text-amber-800 font-medium line-clamp-2">{d.penjelasan_jawaban}</p>
                 </div>
               )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white rounded-[32px] shadow-2xl animate-scale-in">
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Pertanyaan</label>
                <textarea 
                  required 
                  className="w-full h-24 bg-gray-50 border-2 border-gray-100 rounded-3xl p-6 font-medium text-sm text-gray-700 outline-none focus:border-purple-500" 
                  value={form.pertanyaan} onChange={e => setForm({...form, pertanyaan: e.target.value})}
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Jawaban Paling Tepat / Ideal</label>
                <textarea 
                  required 
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Penjelasan Ekstra (Opsional - Diucapkan oleh AI)</label>
                <textarea 
                  className="w-full h-20 bg-amber-50 border-2 border-amber-100 rounded-3xl p-6 font-medium text-sm text-amber-800 outline-none focus:border-amber-500" 
              ...
              >
                ...
                {editingId ? 'Simpan Perubahan' : 'Tambah Modul Wawancara'}
              </button>
            </form>
          </div>
        </div>
