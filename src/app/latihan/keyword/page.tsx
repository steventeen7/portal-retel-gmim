...
import { 
  ...
  BarChart3, Target, Trophy, Play, History, RotateCcw,
  Zap, ArrowRight
} from 'lucide-react';
...
export default function KeywordPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      if (user && user.role !== 'admin' && !user.permissions?.includes('keyword')) {
        ...
        <div className="bg-gray-50/50 p-8 md:p-12 border-b border-gray-100">
           <div className="flex flex-col md:flex-row items-center justify-center gap-6">
              {selected.length > 0 ? (
                <>
                  {selected.map((word, i) => (
                    <div key={i} className="flex flex-col items-center gap-4 animate-scale-in" style={{ animationDelay: `${i * 0.15}s` }}>
                       ...
                       {i < 2 && <ArrowRight className="hidden md:block w-6 h-6 text-gray-200" />}
                    </div>
                  ))}
                </>
              ) : (
        ...
        <div className="grid md:grid-cols-2 gap-8 animate-fade-in-up">
           ...
           <div className="bg-indigo-900 rounded-[40px] p-8 md:p-10 border border-indigo-800 shadow-xl text-white relative flex flex-col">
              <div className="space-y-4 flex-1">
                 ...
                 <h3 className="text-2xl font-black tracking-tight tracking-tight uppercase">Catatan Juri AI</h3>
